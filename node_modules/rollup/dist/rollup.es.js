/*
	Rollup.js v0.51.3
	Fri Nov 10 2017 08:05:09 GMT+0100 (CET) - commit 6d0e492d6a30318495acfa299cb02631796075ce
...
class Variable {
	 ...
	 */
	bindAssignmentAtPath ( path$$1, expression, options ) {}
...
class VariableShapeTracker {
	...
	forEachAssignedToPath ( path$$1, callback ) {
		if ( path$$1.length > 0 ) {
			var nextPath = path$$1[0];
			var remainingPath = path$$1.slice(1);
			if ( this._assignments.has( nextPath ) ) {
	...
	someAtPath ( path$$1, predicateFunction ) {
		var nextPath = path$$1[0];
		var remainingPath = path$$1.slice(1);
		return Array.from( this._assignments.get( SET_KEY ) ).some( assignment => predicateFunction( path$$1, assignment ) )
			|| (
				path$$1.length > 0
				&& (
					(nextPath === UNKNOWN_KEY
						&& Array.from( this._assignments ).some( ( ref ) => {
							var subPath = ref[0];
							var assignment = ref[1];
...
class LocalVariable extends Variable {
	...
	bindAssignmentAtPath ( path$$1, expression, options ) {
		if ( path$$1.length > MAX_PATH_LENGTH || this.boundExpressions.hasAtPath( path$$1, expression ) ) { return; }
	...
	forEachReturnExpressionWhenCalledAtPath ( path$$1, callOptions, callback, options ) {
		if ( path$$1.length > MAX_PATH_LENGTH ) { return; }
	...
	hasEffectsWhenAccessedAtPath ( path$$1, options ) {
		return path$$1.length > MAX_PATH_LENGTH
...
class AssignmentPattern extends Node$1 {
	...
	bindAssignmentAtPath ( path$$1, expression, options ) {
		path$$1.length === 0
		&& this.left.bindAssignmentAtPath( path$$1, expression, options );
	}
...
class CallExpression extends Node$1 {
	bindAssignmentAtPath ( path$$1, expression, options ) {
		...
		&& this.callee.forEachReturnExpressionWhenCalledAtPath( [], this._callOptions, innerOptions => node =>
			node.bindAssignmentAtPath( path$$1, expression, innerOptions.addAssignedReturnExpressionAtPath( path$$1, this ) ), options );
	}
...
class ConditionalExpression extends Node$1 {
	bindAssignmentAtPath ( path$$1, expression, options ) {
		path$$1.length > 0
		&& this._forEachRelevantBranch( node => node.bindAssignmentAtPath( path$$1, expression, options ) );
	}
	...
	bindAssignmentAtPath ( path$$1, expression, options ) {
		if ( path$$1.length > 0 ) {
			if ( path$$1[ 0 ] >= 0 && this._parameters[ path$$1[ 0 ] ] ) {
				this._parameters[ path$$1[ 0 ] ].bindAssignmentAtPath( path$$1.slice( 1 ), expression, options );
			}
		}
	}
...
class Identifier extends Node$1 {
	bindAssignmentAtPath ( path$$1, expression, options ) {
		...
		this.variable
		&& this.variable.bindAssignmentAtPath( path$$1, expression, options );
	}
...
class MemberExpression extends Node$1 {
	...
	bindAssignmentAtPath ( path$$1, expression, options ) {
		...
		if ( this.variable ) {
			...
			this.object.bindAssignmentAtPath( [ this._getPathSegment() ].concat( path$$1 ), expression, options );
		}
	}
...
class ObjectExpression extends Node$1 {
	bindAssignmentAtPath ( path$$1, expression, options ) {
		...
		&& properties.forEach( property => (path$$1.length > 1 || property.kind === 'set')
			&& property.bindAssignmentAtPath( path$$1.slice( 1 ), expression, options ) );
	}
...
class ObjectPattern extends Node$1 {
	bindAssignmentAtPath ( path$$1, expression, options ) {
		path$$1.length === 0
		&& this.properties.forEach( child => child.bindAssignmentAtPath( path$$1, expression, options ) );
	}
...
class Property extends Node$1 {
	bindAssignmentAtPath ( path$$1, expression, options ) {
		if ( this.kind === 'get' ) {
			...
			this.value.bindAssignmentAtPath( path$$1, expression, options );
		}
	}
...
class VariableDeclarator extends Node$1 {
	bindAssignmentAtPath ( path$$1, expression, options ) {
		this.id.bindAssignmentAtPath( path$$1, expression, options );
	}
